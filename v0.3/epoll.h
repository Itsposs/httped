...
class Epoll
{
	public:
		...
		static int epoll_add(int fd, std::shared_ptr<requestData> request, uint32_t events);
		static int epoll_mod(int fd, std::shared_ptr<requestData> request, uint32_t events);
		static int epoll_del(int fd, uint32_t events);
